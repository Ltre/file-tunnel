...
当前基线的应用壳版本为 `instant-tunnel-v63`，并缓存了新增的 `/client/disk-collaboration.js`。
...
260924 人工结果为“勉强验收通过”；260925 后代码继续修正平板/宽屏高度模型。

当前宽屏布局由 `#appShell` 约束在 `100dvh`：

- topbar 占固定高度；
- 主三栏 `.main-layout` 使用剩余 flex 空间；
- `min-height:0` 避免子面板把 shell 撑高；
- 顶部/底部只保留少量留白；
- 各栏内部继续独立纵向 scroll。

左栏 `.qr-section` 的滚动条缩到约 4px、半透明灰白。摄像头、语音聊天、全局对讲使用专门的 `.media-control-btn` 尺寸，不再通过改全局 `.btn` 牵连其它按钮。

这轮修改解决的是“平板宽屏下页面继续向下滚并露出空白”的布局根因，但真实 Android 平板视觉/动态地址栏仍需要实机验收，所以不能写成完全稳定。
...
260925 的高度溢出/底部空白修复已经合入当前基线，核心是由 `#appShell` 管理整个 `100dvh` 高度而不是继续给 `.main-layout` 单独套容易叠加外边距的 clamp 高度。真实 Android 平板仍未完成实机验收。

### 8.1 宽屏顶栏几何对齐

260926 修正了宽屏 topbar 左内边距：不再固定写死 14px，而是按与最大宽度 1400px 的主 `.container` 相同几何公式计算，使刷新按钮左边缘与三栏内容左边缘对齐。

这属于视觉几何一致性修复，不改变移动端 topbar 布局。
