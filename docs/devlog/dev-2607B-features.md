...
## 2026-08-03：多设备上传进度归并修正

### 问题定位

- 多源传输会把一个文件拆为多个分片，每个分片分别上报 `0% -> 100%`；进度界面此前却把同一文件面向所有接收设备的多源上传统一合并成一条记录。
- 两台以上接收设备同时拉取时，不同目标设备以及 P2P/Socket.IO relay 分片会相互覆盖状态，表现为同一文件的进度条反复闪现、短暂消失或链路文案来回变化。
- 这属于供源端进度 UI 的身份归并和隐藏时机问题，不代表文件被其它任务取消，也不应通过修改 ICE、P2P 超时或 relay 策略处理。

### 实现调整

- 多源上传进度改为按“文件 + 目标设备”归并：同一目标设备的连续分片及其 P2P 到 relay 兜底仍复用同一条进度，不同目标设备不再互相覆盖。
- 多源上传状态增加目标设备名称，三设备场景可直接区分具体哪一对设备采用 P2P 或 relay。
- 单个多源分片完成后的隐藏等待由通用的 800ms 调整为多源上传专用的 2500ms；后续分片开始时会取消待执行的隐藏，减少连续分片之间的闪烁。其他传输类型仍保持原来的 800ms。
- 未修改 `client/file-assets.js`、ICE candidate、1500ms P2P 等待、并发数、分片大小或 Socket.IO relay 降级流程。

### 回归验证

- 新增多目标设备进度 key 回归测试，确认同一目标的 P2P/relay 分片保持一条记录，而不同目标设备使用不同记录。
- `npm run test:p2p` 共 17 项通过；`app.js` 通过语法检查。

## 2026-08-04：多源分片重复传输与供源端进度修正

### 日志结论

- 供源端此前把每个 2 MiB 分片各自的 `0% -> 100%` 直接写入按“文件 + 目标设备”归并的同一条进度，因此正常的下一分片也会造成 `98% -> 35%` 一类倒退。
- 实机日志同时确认存在真实重复请求：同一文件、目标设备和 `part-N` 分片会出现带递增重试序号的请求。接收端在分片字节已经完整收齐、但完成帧尚在消息队列中等待处理时，12 秒 watchdog 仍可能把它误判为停滞并清零重派。
- 整个多源文件进入持久化阶段后，迟到的已完成分片请求此前仍可能通过初步接收检查，造成文件已经可用后供源端又短暂上传。

### 生命周期修正

- 分片字节达到期望长度后进入“等待完成帧”状态；此时不再套用 12 秒数据停滞阈值，改用原有 30 秒接收超时兜底。完成帧真正丢失时仍会重试，不会无限等待。
- 整文件落库期间保留多源传输上下文并标记 `completing`；已完成分片和正在落库文件的迟到请求会被明确拒绝，避免再次灌入相同数据。
- 落库失败时释放完成上下文并进入既有下载重试流程，避免因新增完成态而永久卡住。
- 未修改 ICE、1500ms P2P 等待、DataChannel 参数、分片大小、上传/下载并发数或 Socket.IO relay 降级策略。

### 进度与窄屏 UI

- 供源端多源进度按“文件 + 目标设备 + 唯一分片”累计已发送字节；同一分片重试只取最大已发送量，不重复计数，也不会令进度倒退。
- 单个分片完成后继续沿用短暂隐藏机制；后续分片到来会取消隐藏并沿用累计进度，不再把每个分片伪装成整个文件的 100%。
- 进度标题改为两列网格，在窄屏上为左侧方向图标和文件名保留最小宽度；右侧链路及目标设备文案超长时省略，避免 Android 10 上文件名只剩半个字。

### 回归验证

- 新增“完整分片等待完成帧不被 12 秒 watchdog 清零”“重试分片不重复累计或倒退”“落库阶段拒绝迟到分片”回归用例。
- `npm run test:p2p` 共 20 项通过；`client/file-assets.js` 与 `app.js` 通过语法检查。
