...
从用例1~12来看，windows设备A不论有没有开启代理或代理开什么模式，都不影响测试表现，而取决于android设备B。
从用例13~16来看，两台相似环境都使用igniter的android设备，只要不全部都启用igniter且启用[Exempt Chinese Domain/IPs]，那么就可以走P2P。
