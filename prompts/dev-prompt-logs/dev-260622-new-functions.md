...
【OK】(6) admin管理页隧道列表：应该首先列出所有在线的会话（不论有没有聊天记录），其次列出离线但存在聊天记录的会话。至于之前已经实现的活跃/非活跃状态，可以继续标记在隧道列表中。
...
7.6 
问：
【基本OK，不再重复生成文件副本缓存，但是会话内的简易资源浏览器还没开发】(1) 测试了协同编辑区域的“引用文件”功能，发现上传文件后确实可以在引用下拉列表中找到。
...
7.7
问：
【勉强OK，可能还是存在中断传输的缓存】(1) 在页面合适的角落提供清理垃圾按钮：已经确认可以清理游离态的文件缓存（即没有被文件传输历史或富文本关联的文件），不过还不确定能否清理中断传输的文件缓存

7.8
问：
【OK】(1) 发现隧道暗号（短码）怎么一直被初始化为5个数字，不是同时支持数字或字母(不区分大小写)吗
...
7.11
【OK】问：
如果聊天记录中删除某个文件，而这个文件被某个富文本引用的话，那时就应该创建这个文件的副本确保富文本不会有异常，或者就干脆让这个文件处于游离态，不管怎么做，会话内的简易资源浏览器要提上日程了，文件名、大小、格式，最重要的是要给文件标记引用的位置（焦点闪烁跳转到聊天记录锚点，或者协同编辑框）
请开发这个建议的资源浏览器，支持管理文件。


答：
我会把它做成会话内的资源浏览器，而不是单纯列出 IndexedDB：资源要能显示名称、大小、类型、缓存状态、引用位置，并能定位到聊天记录或协同编辑器。删除聊天记录时，文件将作为“仍被引用的资源”保留，浏览器可统一管理清理。
资源浏览器会以“文件资源”为中心，不依赖某一条聊天记录：同一资源会聚合所有聊天、富文本和协同编辑引用。被引用的资源不能从浏览器彻底删除；未引用的资源才可本地移除，避免误伤协同编辑里的图片。
编辑了 1 个文件
我现在落地浏览器本体：它会汇总 IndexedDB 的文件缓存和消息/编辑器中的资源引用，并提供定位、下载、清除本机缓存、还原和清除无引用资源。引用中的图片资源会明确标为受保护，避免误删。
编辑了 1 个文件
浏览器和管理动作已经接入，资源会聚合显示并允许直接跳回对应聊天记录或编辑器元素。接下来我做一次真实页面检查，确认弹窗、筛选和动作按钮没有被现有页面样式挤坏。
Used 浏览器
静态检查已经通过。当前机器没有正在监听的本地服务，我会临时启动工作区里的服务进行页面验证；不会改动任何运行配置。
本地端口 80 已被现有服务占用，浏览器自动连接也没有及时返回，因此我不拿它做危险的端口切换。代码层面继续做可执行的资源关系单测，确保聊天文件、富文本图片副本和编辑器引用会被正确聚合。
聚合单测通过。我再收紧一个边界：当原聊天文件已被删除、只剩独立图片副本时，不把不存在的“原文件”虚构成一项资源；浏览器只展示实际仍可管理的资源。
编辑了 1 个文件
实现已收束，接着跑最后一轮语法、差异和资源关系检查，并确认只改动了这次需要的页面与前端逻辑。之后我会直接给你可测试的结果，不再停在中间状态。
已完成资源浏览器，改动仅在 [app.js (line 3119)](D:\\Workspace\\github\\Ltre\\file-tunnel\\app.js:3119) 和 [index.html (line 472)](D:\\Workspace\\github\\Ltre\\file-tunnel\\index.html:472)，均未暂存、未提交。
页面设备区新增“资源浏览器”。它会按资源聚合显示文件名、大小、格式、缓存状态、传输中断状态，以及聊天文件、富文本、协同编辑器中的全部引用位置。
每个引用位置都可点击定位：聊天记录会滚动并高亮对应消息，协同编辑器会聚焦并闪烁对应图片或文件引用。图片引用继续使用独立的编辑器资源副本，删除原聊天文件不会影响已插入富文本的图片。
资源管理支持：
下载本机缓存资源
清除本机缓存并保留引用
从在线设备还原文件或图片
...
7.11.1
【OK】问：
弹出的资源管理器位于中央遮罩层，我觉得体验不好。
点击某个文件的引用来源，遮罩层就关掉，并跳转到聊天记录某个锚点。
不如将这个资源管理器移动到“连接设备”的上层，百分百遮住“连接设备”，这样就不用在定位资源引用锚点时被迫关闭资源管理器了
答：
已调整完成。
资源浏览器现在覆盖“连接设备”整个左侧面板，不再使用中央遮罩层；打开后会完全遮住设备信息，关闭按钮留在该面板右上角。
点击任意“引用位置”后，资源浏览器保持打开，聊天记录或协同编辑器会滚动并闪烁高亮对应锚点，便于连续检查多个引用。

问：
【OK】8、在设备A开启摄像头后，仅在A将按钮文字改成“关闭摄像头”即可，而在别的设备则将按钮显示为“顶号开播”，当A关闭摄像头后，别的设备得摄像头按钮文字还原为“摄像头”
...
问：
【OK】9、测试了全局对讲机，确实还没发现问题，且打开后有“关闭对讲机”按钮。但是，针对某个设备的一对一对讲机，点击其右侧”对讲机“按钮后，其按钮文字没有变更为“关闭对讲”，不过功能是正常的，实际是可以关闭的。
...
10、
【OK】问：Android通过PWA分享文件测试如何？
答：已经实现，支持同时将多个文件分享到PWA，邀请朋友加入同一个隧道，朋友即可收到隧道内的所有信息。
todo：要注意及时刷新前端资源链接
