...
test('真实 HTTP 网盘 API：原手机路径、异步操作、移动重命名、下载、幂等完成、部分删除与隔离', async t => {
    ...
    const captionCount = syncedCaptions.length;
    failCaption = true;
    const renamedFolder = await request('/directories', { method: 'PATCH', ...json({ path: 'new/deep', name: 'nested' }) });
    assert.equal((await wait(renamedFolder.operation_id)).status, 'completed');
    assert.equal(syncedCaptions.length, captionCount);
    assert.equal(api.spaces.get('music').get(user.id, file.id).captionSyncPending, false);
    const failedRename = await request('/files/' + file.id, { method: 'PATCH', ...json({ name: 'renamed-2.txt' }) });
    assert.equal((await wait(failedRename.operation_id)).errorCode, 'TELEGRAM_CAPTION_SYNC_PENDING');
    ...
    assert.equal(syncedCaptions.at(-1).name, 'renamed-2.txt');
    assert.equal(api.spaces.get('music').get(user.id, file.id).captionSyncPending, false);
    ...
    assert.equal((await request('/files/' + file.id)).name, 'renamed-2.txt');
    failDelete = false;
...
test('Telegram transport 按 10 个拆 album，字节进度不含 multipart 头，异常不泄露 token', async t => {
    ...
    assert.ok(captions.every(item => !/(?:^|\n)path:/.test(item.caption)));
...
test('后台按来源/用户/分区审计文件，屏蔽立即阻断分享，审核删除保留可由用户清理的占位', async t => {
    ...
    const add = async (name, sourceAppId, folderPath = '') => {
        const upload = drive.begin({ owner: user, folderPath, files: [{ name, size: 3, type: 'text/plain' }], maxDepth: 20, sourceAppId });
    ...
    const first = await add('待审核-a.txt', 'music-app'), second = await add('待审核-b.txt', 'music-app'), directoryFile = await add('目录内文件.txt', 'music-app', '待审核目录');
    ...
    assert.equal(system.users[0].userId, user.id); assert.equal(system.users[0].spaces[0].fileCount, 3);
    ...
    assert.deepEqual(new Set(reviews.files.map(file => file.id)), new Set([first.id, second.id, directoryFile.id]));
    assert.equal(await (await fetch(base + `/admin/files/${directoryFile.id}/download?user_id=${user.id}`)).text(), 'abc');
    ...
    assert.equal((await fetch(base + '/admin/reviews/' + first.id, { method: 'PATCH', ...json({ user_id: user.id, action: 'unblock' }) })).status, 200);
    assert.equal(drive.get(user.id, first.id).reviewStatus, '');
    assert.equal((await fetch(base + '/admin/directories/review', { method: 'PATCH', ...json({ user_id: user.id, path: '待审核目录', action: 'block' }) })).status, 200);
    assert.equal(drive.get(user.id, directoryFile.id).reviewStatus, 'blocked');
    assert.equal((await fetch(base + '/admin/directories/review', { method: 'PATCH', ...json({ user_id: user.id, path: '待审核目录', action: 'unblock' }) })).status, 200);
    assert.equal((await fetch(base + '/admin/directories/review', { method: 'PATCH', ...json({ user_id: user.id, path: '待审核目录', action: 'delete' }) })).status, 200);
    assert.equal(drive.get(user.id, directoryFile.id).reviewStatus, 'deleted');
    assert.equal((await fetch(base + '/admin/reviews/' + second.id, { method: 'PATCH', ...json({ user_id: user.id, action: 'delete' }) })).status, 200);
    assert.equal(removals, 2); assert.equal(drive.get(user.id, second.id).reviewStatus, 'deleted'); assert.equal(drive.get(user.id, second.id).fileId, '');
    ...
    assert.equal(removals, 2); assert.ok(deletion.operation_id);
});
