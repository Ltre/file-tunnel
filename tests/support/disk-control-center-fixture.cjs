...
async function run() {
    ...
    try {
        ...
        const calls = {};
        for (const id of ['mobileForceRefreshBtn', 'magnetCacheBtn', 'cycleThemeBtn', 'tunnelSettingsBtn', 'leaveTunnelPanelBtn', 'scanTunnelCodeBtn', 'receiveLightBtn', 'resourceBrowserBtn', 'webWorkshopBtn', 'notificationCenterBtn', 'telegramDriveBtn']) document.getElementById(id).addEventListener('click', () => { calls[id] = (calls[id] || 0) + 1; });
        await showJoinedSessionSwitcher(); let center = document.querySelector('.control-center-overlay');
        assert(center.querySelectorAll('[data-control-tile]').length === 11, '控制中心磁贴不完整');
        ...
        center.querySelector('[data-control-tile="magnet"]').click(); assert(!center.isConnected && calls.magnetCacheBtn === 1, '磁链入口未调用或没有关闭控制中心'); results.push('十一项磁贴、主题原地轮换、磁链关闭及面板高度');
        await showJoinedSessionSwitcher(); center = document.querySelector('.control-center-overlay');
