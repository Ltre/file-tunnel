...
async function run() {
    ...
    try {
        ...
        const calls = {};
        for (const id of ['mobileForceRefreshBtn', 'magnetCacheBtn', 'cycleThemeBtn', 'tunnelSettingsBtn', 'leaveTunnelPanelBtn', 'scanTunnelCodeBtn', 'receiveLightBtn', 'resourceBrowserBtn', 'telegramDriveBtn']) document.getElementById(id).addEventListener('click', () => { calls[id] = (calls[id] || 0) + 1; });
