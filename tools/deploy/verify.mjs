...
function shouldExistAsStaticFile(ref) {
  ...
  if (/^\/(?:admin|tgbot|sns-cookies|youtube-premium-dl|downloader|downloadList|record|magnet|device|light-file-parts)\b/.test(ref)) return false;
  return true;
}

async function checkJavaScriptSyntax(distRoot, relativePath) {
...
async function main() {
  ...
  await checkJavaScriptSyntax(distRoot, 'server.js');
