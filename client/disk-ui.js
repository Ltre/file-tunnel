...
function closeTelegramDriveDialog(result = null, { fromHistory = false, replaceHistory = false } = {}) {
    ...
    dialog.hidden = true;
    document.body.classList.remove('telegram-drive-dialog-open');
...
function openTelegramDriveDialog({ title, body, confirmText = '确定', confirmClass = 'btn-primary', cancelText = '取消', validate, historyEntry = false, dismissOnBackdrop = false } = {}) {
    ...
    dialog.hidden = false;
    document.body.classList.add('telegram-drive-dialog-open');
...
async function inviteDiskCollaboration(item) {
    ...
    const mark = collection => { for (const entry of [...(collection?.folders || []), ...(collection?.files || [])]) if (item.kind === entry.kind && (item.kind === 'directory' ? entry.path === item.path : entry.id === item.id)) entry.collaborationId = collaborationId; };
    item.collaborationId = collaborationId;
    mark(telegramDriveCurrentData); mark(telegramDriveSearchData);
    renderTelegramDriveItems();
    await refreshDiskCollaborations();
    refreshTelegramDriveContents().catch(() => {});
...
async function manageDiskCollaboration(entry, trigger = document.getElementById('telegramDriveCollaborationManageBtn')) {
    ...
    const render = async () => {
        ...
        const addInvite = document.createElement('button'); addInvite.type = 'button'; addInvite.className = 'btn btn-secondary'; addInvite.textContent = '创建新邀请链接';
        addInvite.onclick = async () => {
            addInvite.disabled = true;
            try { await window.DiskClient.raw('/collaborations/invitations', window.DiskClient.json('POST', current.kind === 'directory' ? { kind: 'directory', path: current.path } : { kind: 'file', fileId: current.fileId })); await render(); }
            catch (error) { addInvite.disabled = false; alert(telegramDriveErrorText(error)); }
        };
        body.append(addInvite);
        ...
        for (const member of current.members || []) {
            const row = document.createElement('div'); row.className = 'disk-collaboration-row';
            const person = current.memberDetails?.find(detail => detail.id === member);
            const name = document.createElement('span'); name.className = 'disk-collaboration-member';
            name.textContent = [person?.name || person?.username || '网盘用户', person?.telegramId ? `Telegram ID：${person.telegramId}` : '', person?.username ? `Passkey 别名：${person.username}` : '', `账号 ID：${member}`].filter(Boolean).join(' · ');
...
function renderTelegramDriveContextMenu(item, anchor, actions) {
    ...
    requestAnimationFrame(() => {
        ...
        const availableWidth = Math.max(1, rightEdge - leftEdge - 16), availableHeight = Math.max(1, bottomEdge - topEdge - 16);
        const width = Math.min(Math.max(190, menu.scrollWidth || menu.offsetWidth || 190), availableWidth);
        menu.style.width = `${width}px`;
        menu.style.maxWidth = `${availableWidth}px`;
        menu.style.maxHeight = `${availableHeight}px`;
        const height = Math.min(menu.scrollHeight || menu.offsetHeight || 1, availableHeight);
...
function beginTouchDiskDrag(items, sourceRow, event) {
    if (!items.length) return;
    if (diskPointerDragActive) return;
    diskPointerDragActive = true;
    try { sourceRow.setPointerCapture?.(event.pointerId); } catch (_) {}
    ...
    let target = null, scrollFrame = 0, lastPoint = event;
    ...
    const escape = keyEvent => { if (keyEvent.key === 'Escape') { keyEvent.preventDefault(); keyEvent.stopImmediatePropagation(); cleanup(); } };
    const autoScroll = () => {
        const list = document.getElementById('telegramDriveList'), rect = list.getBoundingClientRect();
        if (lastPoint.clientX >= rect.left && lastPoint.clientX <= rect.right) {
            const speed = lastPoint.clientY < rect.top + 36 ? -7 : lastPoint.clientY > rect.bottom - 36 ? 7 : 0;
            if (speed) { list.scrollTop += speed; position(lastPoint); }
        }
...
function installDiskPointerDrag(row, item) {
    ...
    row.addEventListener('pointermove', event => {
        ...
        if (start.type === 'touch') {
            ...
            if (telegramDriveSelected.size && !telegramDriveSelected.has(telegramDriveItemKey(item))) return;
            event.preventDefault(); beginTouchDiskDrag(items, row, event); start = null;
        }
    });
    const finish = event => {
...
function initDiskEnhancements() {
    ...
    overflowButton.onclick = event => {
        ...
        document.body.append(overflowMenu); overflowMenu.hidden = false;
        const anchor = overflowButton.getBoundingClientRect();
        overflowMenu.style.left = `${Math.max(8, anchor.left - overflowMenu.offsetWidth - 7)}px`;
        overflowMenu.style.top = `${Math.max(8, Math.min(anchor.top, innerHeight - overflowMenu.offsetHeight - 8))}px`;
...
function init(options = {}) {
    ...
    document.getElementById('telegramDriveBottomMenuBtn')?.addEventListener('click', event => {
        ...
        if (!telegramDriveSelected.size) showTelegramDriveBackgroundMenu(anchor);
        else showTelegramDriveItemMenu([...telegramDriveSelected.values()][0], anchor).catch(error => alert(telegramDriveErrorText(error)));
    });
...
window.DiskUI = { init, open: openTelegramDrive, close: closeTelegramDrive, ownsHistory: ownsTelegramDriveHistory, upload: uploadFilesToTelegramDrive, render: renderTelegramDrive, prompt: promptTelegramDriveText,
    async chooseDirectory(options = {}) {
        ...
        const memoryKey = 'telegramDriveSaveDirectory:' + (options.userId || 'browser');
        let initialPath = telegramDrivePath;
        try { const saved = localStorage.getItem(memoryKey); if (saved !== null) initialPath = saved; } catch (_) {}
        const path = await chooseTelegramDriveDestination([], { title: options.title || '选择网盘目录', confirmText: options.confirmText || '选择此目录', onCreateDirectory: options.onCreateDirectory, initialPath });
        if (path !== null) { try { localStorage.setItem(memoryKey, path); } catch (_) {} }
        return path;
    },
