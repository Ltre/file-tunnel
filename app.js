...
function initUI() {
    ...
    document.getElementById('telegramDriveBtn')?.addEventListener('click', () => {
        document.getElementById('connectionHeaderMenu').hidden = true;
        window.DiskUI.open().catch(error => alert(error.message));
    });
    document.addEventListener('click', event => {
        if (!event.target.closest?.('.connection-header-menu-wrap')) document.getElementById('connectionHeaderMenu').hidden = true;
    });
    configureLightTransfer();
    ...
    document.getElementById('mediaFullscreenViewer')?.addEventListener('pointercancel', event => {
        if (mediaFullscreenMousePan?.id === event.pointerId) { mediaFullscreenMousePan = null; document.getElementById('mediaFullscreenContent')?.classList.remove('mouse-panning'); }
        mediaFullscreenGesture.pointers.delete(event.pointerId);
